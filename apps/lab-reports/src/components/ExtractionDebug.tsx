...
export const ExtractionDebug: React.FC<ExtractionDebugProps> = ({ fileContent, fileName }) => {
  ...
  return (
    <div className="mt-4 p-4 bg-warm-100 border-2 border-warm-300 rounded-lg text-sm font-mono">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-bold text-warm-700">🔍 Extraction Debug Info</h3>
        <span className="text-xs px-2 py-1 bg-warm-200 rounded">{fileName}</span>
      </div>

      <div className="grid grid-cols-2 gap-4">
        {/* Extraction Method */}
        <div className="col-span-2 p-2 bg-white rounded">
          <div className="text-xs text-warm-500 mb-1">Extraction Method</div>
        ...
        <div className="p-2 bg-white rounded">
          <div className="text-xs text-warm-500 mb-1">Text Content</div>
          <div className="space-y-1">
            <div>
              <span className="text-warm-600">Length:</span>{' '}
              <span className="font-semibold">{debug.textLength.toLocaleString()} chars</span>
            </div>
            <div>
              <span className="text-warm-600">Lines:</span>{' '}
        ...
        {/* Image Statistics */}
        <div className="p-2 bg-white rounded">
          <div className="text-xs text-warm-500 mb-1">Images</div>
          <div className="space-y-1">
            <div>
              <span className="text-warm-600">Count:</span>{' '}
              <span className="font-semibold">{debug.imageCount}</span>
            </div>
            <div>
              <span className="text-warm-600">Total size:</span>{' '}
              <span className="font-semibold">{(debug.totalImageSize / 1024).toFixed(0)} KB</span>
            </div>
            <div>
              <span className="text-warm-600">Avg size:</span>{' '}
        ...
        {/* Text Structure */}
        <div className="col-span-2 p-2 bg-white rounded">
          <div className="text-xs text-warm-500 mb-1">Text Structure</div>
        ...
        {/* Table Detection */}
        <div className="col-span-2 p-2 bg-white rounded">
          <div className="text-xs text-warm-500 mb-1">Table Detection</div>
          <div className="grid grid-cols-2 gap-2">
            <div>
            ...
            <div>
              ...
              {debug.tableDetection?.hasTableStructure ? (
                ...
                <span className="font-semibold text-warm-400">✗ No</span>
              )}
            </div>
          </div>
          {debug.tableDetection?.hasTableStructure && (
        ...
        {/* Text Sample */}
        <div className="col-span-2 p-2 bg-white rounded">
          <div className="text-xs text-warm-500 mb-1">Text Sample (first 500 chars)</div>
          <div className="text-xs bg-warm-50 p-2 rounded border border-warm-200 max-h-32 overflow-y-auto whitespace-pre-wrap break-words">
