          ...
          <div className="bg-white rounded-lg shadow-xs p-4 mb-6">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <div className="flex items-center gap-2">
                <label htmlFor="text-size-select" className="text-sm">
                  {t('accessibility.textSize', 'Leturstærð')}:
                </label>
                <select
                  id="text-size-select"
              ...
              <div className="bg-white rounded-lg shadow-md p-8">
                ...
                <div className="bg-warm-50 rounded-lg p-4 mb-6">
                  <p className="text-sm text-warm-600">
                    {t('progress.problemsCompleted', 'Verkefni kláruð')}:{' '}
                ...
                <div className="grid gap-4">
                  ...
                  >
                    ...
                    <p className="text-sm text-green-200 mt-2">
                      {t('menu.challenges')}:{' '}
                      {progress.levelProgress?.level1?.questionsAnswered || 0}/6
                    </p>
                  </button>
                  ...
                  >
                    ...
                    {progress.levelProgress?.level1?.mastered ? (
                      <p className="text-sm text-blue-200 mt-2">
                        {t('menu.problems')}:{' '}
                        {progress.levelProgress?.level2?.problemsCompleted || 0}/15
                      </p>
                    ) : (
                  ...
                  <button
                    ...
                    style={{
                      backgroundColor: !progress.levelProgress?.level2?.mastered
                        ? undefined
                        : '#f36b22',
                    }}
                  >
                    ...
                    {progress.levelProgress?.level2?.mastered ? (
                      <p className="text-sm text-orange-200 mt-2">
                        {t('menu.problems')}:{' '}
                        {progress.levelProgress?.level3?.problemsCompleted || 0}/10
                      </p>
                    ) : (
            ...
            <>
              <Level3
                onComplete={(levelProgress, maxScore = 1000, hintsUsed = 0) => {
                  ...
                  // Track achievements
                  const avgScore =
                    levelProgress.compositeScores.length > 0
                      ? levelProgress.compositeScores.reduce((a, b) => a + b, 0) /
                        levelProgress.compositeScores.length
                      : 0;
                  const score = Math.round(avgScore * 1000);
                  trackLevelComplete(3, score, maxScore, {
                    hintsUsed: hintsUsed || levelProgress.hintsUsed,
                  });
                  trackGameComplete();
                  playLevelComplete();
                  triggerLevelComplete();
              ...
              <div className="bg-white rounded-lg shadow-md p-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-b pb-4">
                    ...
                    <p className="text-sm text-warm-600">
                      {t('stats.accuracy')}:{' '}
                      {progress.levelProgress?.level1?.questionsAnswered
                        ? Math.round(
                            (progress.levelProgress.level1.questionsCorrect /
                              progress.levelProgress.level1.questionsAnswered) *
                              100
                          )
                        : 0}
                  ...
                  <div className="border-b pb-4">
                    ...
                    <p className="text-sm text-warm-600">
                      {t('menu.problems')}: {progress.levelProgress?.level2?.problemsCompleted || 0}
                      /15
                    </p>
                    <p className="text-sm text-warm-600">
                      {t('stats.predictionAccuracy')}:{' '}
                      {progress.levelProgress?.level2?.predictionsMade
                        ? Math.round(
                            (progress.levelProgress.level2.predictionsCorrect /
                              progress.levelProgress.level2.predictionsMade) *
                              100
                          )
                        : 0}
                  ...
                  <div className="pb-4">
                    ...
                    <p className="text-sm text-warm-600">
                      {t('menu.problems')}: {progress.levelProgress?.level3?.problemsCompleted || 0}
                      /10
                    </p>
                    <p className="text-sm text-warm-600">
                      {t('stats.averageScore')}:{' '}
                      {progress.levelProgress?.level3?.compositeScores?.length
                        ? Math.round(
                            (progress.levelProgress.level3.compositeScores.reduce(
                              (a, b) => a + b,
                              0
                            ) /
                              progress.levelProgress.level3.compositeScores.length) *
