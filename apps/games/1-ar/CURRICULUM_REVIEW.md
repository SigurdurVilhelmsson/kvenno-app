...
| Topic                                   | Status | Where                                                                                                                                             |
| --------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| Balancing equations                     | ●      | Stilla efnajöfnur (20 reactions, all verified correct)                                                                                                 |
...
| Reaction patterns (combustion etc.)     | ◐      | Combustion equations appear in Stilla efnajöfnur; reaction _types_ are never taught as a classification                                                |
...
**Relation to the roadmap's four.** `docs/plans/2026-08-16-games-roadmap.md:18` names a different four — electrolytes/precipitation, empirical formula, Ka/Kb, Ksp — and that is the platform-wide planning set. The two are not in conflict: this list is Year-1 scoped. The roadmap folds net ionic into its electrolytes row, tracks percent yield under "Also unplaced" at the end of Phase 5, and adds Ka/Kb and Ksp, which are Year-3 gaps this review never looked at. Use the roadmap's set for planning; use this one for what Year 1 leaves undone.
...
| B12 | Stilla efnajöfnur      | Non-reduced coefficient sets accepted as correct (4H₂ + 2O₂ → 4H₂O passes); the lowest-whole-number requirement is never checked or mentioned                                                                                                                                                                                                                                                                                                                                                                                                                                           | `stilla-efnajofnur/src/utils/balanceChecker.ts:71-74`                                        |
...
|                                       | Einingagr. | Lotukerfið | Nafnakerfið | Mólmassi | Stilla efnajöfnur | Takmarkandi | Lausnir |
| ------------------------------------- | ---------- | ---------- | ----------- | -------- | ------------ | ----------- | ------- |
...
2. **Live, manipulable state.** Stilla efnajöfnur's `AtomCounter` (`AtomCounter.tsx:20-56`) recomputes on every keystroke — the one place a game feels like a machine responding rather than grading. (Caveat below.)
...
- **One shared line hides the explanation in every game in the repo.** `packages/shared/components/FeedbackPanel/FeedbackPanel.tsx:104` is `useState(false)`, and `:186` gates the explanation body on it — so the authored "why" is one click away and off by default at all 26 call sites across all three years. Six games' worth of separately-filed "feedback is binary" findings are this one line. Two scoping caveats, both checked: the _misconception_ slot renders **outside** the toggle (`:202-213`), so games that populate it are not bare — and Mólmassi additionally renders its own visible content on every wrong answer (`Level1.tsx:455` breakdown table, `Level3.tsx:476-487` worked solution). Where the bare word **"Rangt"** really is the whole response is the games that pass only `explanation` and never `misconception`: **Lotukerfið** (zero misconception call sites, all 26 items) and **Stilla efnajöfnur** (its per-element diagnostic is inside the collapsed slot). Note also that two obvious-looking fixes are no-ops: `showExplanation` is already `true` and gates only the toggle button, and there is no `defaultExpanded` prop on `FeedbackPanelConfig`.
- **No game gates any level.** All seven render every level button unconditionally (re-verified 2026-08-17: `grep -rn '\.locked' apps/games/*/*/src --include=*.tsx` returns zero), while **four** of the seven still ship dead "Ljúktu stigi 1 fyrst" strings — Einingagreining (`dimensional-analysis/src/i18n.ts:26,31`), Nafnakerfið (`nafnakerfid/src/i18n.ts:23-24`, under the different key names `completeLevel1First`/`completeLevel2First`), Takmarkandi (`takmarkandi/src/i18n.ts:27,33`) and Lausnir (`lausnir/src/i18n.ts:38,46`). Anyone wiring or stripping the gating must grep both key names. Every "the hardest level has the least scaffolding" finding is worse than filed, because that level is the one a confused student can open first.
...
- **Fixed, exhaustible pools.** Einingagreining 37 items, Stilla efnajöfnur 20, Nafnakerfið ~40 answerable, Lausnir 16 hand-authored. Lotukerfið and Mólmassi L2 generate; nothing else does.
...
- **Stilla efnajöfnur's counter is never gated**, so the intended reasoning is optional: press + toward whichever number is smaller until the rows turn green. The gold-standard reputation belongs to the component, not to the loop it sits in.
...
5. **The three games with zero tests are exactly the three carrying unguarded blockers.** The suite is green — 55 files, 967 tests, 57s — and certifies nothing about chemistry: Lotukerfið, Stilla efnajöfnur and Takmarkandi have no test files at all (Takmarkandi's `__tests__/` is an empty directory), and they hold B1, B8 and B12. This is why four review iterations missed these defects.
...
- **`CLAUDE.md:161`** lists Stilla efnajöfnur as gold standard on the strength of its atom counter. Worth qualifying: the counter is excellent and ungated (§3).
...
Twelve e2e specs already exist in `e2e/` and were not part of any prior review iteration. Open behavioural questions they could settle cheaply: whether Einingagreining's "Áskorun 11 / 10" progress bar visibly overflows; whether Lausnir's dead submit button reads as a broken app; and whether Stilla efnajöfnur's ungated atom counter really does permit hill-climbing to a green table without reasoning — the single most load-bearing engagement claim here, given that CLAUDE.md holds that game up as gold standard.
