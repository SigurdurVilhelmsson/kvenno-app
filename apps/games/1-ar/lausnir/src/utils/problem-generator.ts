...
function generateMolarityFromMassProblem(difficulty: Difficulty, chemical: Chemical): Problem {
  ...
  return {
    ...
    question: `Þú leysir ${n(massInGrams)} g af ${chemical.displayName} (mólmassi ${n(chemical.molarMass)} g/mól) í ${volumeInML} mL af lausn. Hver er mólstyrkurinn?`,
    answer: molarity,
    unit: 'M',
    difficulty: difficulty,
    hints: [
      'Fyrst reiknaðu mól = g / (g/mól), síðan M = mól / L',
...
function generateMassFromMolarityProblem(difficulty: Difficulty, chemical: Chemical): Problem {
  ...
  return {
    ...
    question: `Þú ert með ${volumeInML} mL af ${n(molarity)} M ${chemical.name} lausn. Hversu mörg grömm af ${chemical.name} eru í lausninni? (mólmassi ${n(chemical.molarMass)} g/mól)`,
    answer: mass,
    unit: 'g',
    difficulty: difficulty,
    hints: [
