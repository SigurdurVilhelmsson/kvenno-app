/**
 * Keep the part of the board a student just changed on screen.
 *
 * On a 360 × 740 phone every phase is several screens tall, and each tap
 * changes something far from the finger: a pool card lands in the chain above
 * it, the worked solution grows below the fold one step at a time, and "Næsta
 * dæmi" swaps the problem statement in at the top of the page. Without this the
 * student taps and sees nothing happen. On a desktop screen the change is
 * usually already in view, and then nothing moves.
 */
 ...
 * `through`, so a new chain card and the result it produced come into view
 * together.
 */
