...
function generateChallenge(challengeIndex: number): Challenge {
  ...
  const reaction = LEVEL1_REACTIONS[Math.floor(Math.random() * LEVEL1_REACTIONS.length)];
  ...
  const renderOptions = () => {
    switch (challenge.type) {
      ...
      case 'count_excess': {
        ...
        options.add(Math.floor(correctAnswer / 2));

        // Remove zero/negative values (meaningless in chemistry)
        const validOptions = Array.from(options).filter((n) => n > 0);
        while (validOptions.length < 4) {
          validOptions.push(correctAnswer + 2);
        }

        const shuffledNumericOptions = shuffleArray(validOptions.slice(0, 4));
