...
export const LEVEL1_CHALLENGES: Level1Challenge[] = [
  {
    ...
    hints: {
      ...
      strategy:
        'Berðu saman markmiðs-pH og pKa. Þegar þau eru jöfn, hvað þýðir það fyrir hlutfallið?',
      method:
        'pH = pKa + log([Basi]/[Sýra]). Þegar pH = pKa, þá er log([Basi]/[Sýra]) = 0, sem þýðir [Basi]/[Sýra] = 1.',
      solution:
        'Markmiðs-pH (4.74) = pKa (4.74). Þess vegna þarftu JAFNT af sýru og basa. Settu 5 af hvoru → hlutfall = 1.0.',
    },
    hintsEn: {
      topic:
        'This is about the Henderson-Hasselbalch equation and the relationship between pH and pKa.',
      strategy:
        'Compare the target pH and pKa. When they are equal, what does that mean for the ratio?',
      method:
        'pH = pKa + log([Base]/[Acid]). When pH = pKa, then log([Base]/[Acid]) = 0, which means [Base]/[Acid] = 1.',
      solution:
        'Target pH (4.74) = pKa (4.74). Therefore you need EQUAL amounts of acid and base. Add 5 of each → ratio = 1.0.',
    },
    hintsPl: {
      ...
      method:
        'pH = pKa + log([Zasada]/[Kwas]). Gdy pH = pKa, to log([Zasada]/[Kwas]) = 0, co oznacza [Zasada]/[Kwas] = 1.',
      solution:
    ...
    explanationPl:
      'Gdy kwas i zasada są w równowadze (1:1), pH jest dokładnie równe pKa. To jest punkt środkowy buforu!',
  },
  {
    id: 2,
    system: 'CH₃COOH / CH₃COO⁻',
    ...
    hints: {
      ...
      method:
        'pH = pKa + log([Basi]/[Sýra]). 5.0 = 4.74 + log([Basi]/[Sýra]). log([Basi]/[Sýra]) = 0.26.',
    ...
    hintsEn: {
      ...
      method:
        'pH = pKa + log([Base]/[Acid]). 5.0 = 4.74 + log([Base]/[Acid]). log([Base]/[Acid]) = 0.26.',
    ...
    hintsPl: {
      ...
      method:
        'pH = pKa + log([Zasada]/[Kwas]). 5.0 = 4.74 + log([Zasada]/[Kwas]). log([Zasada]/[Kwas]) = 0.26.',
      solution: 'Proporcja [Zasada]/[Kwas] ≈ 1.8. Na przykład: 9 zasady i 5 kwasu → 9/5 = 1.8.',
    },
    explanation:
      'Til að hækka pH yfir pKa þarf meira af basa. Hlutfallið [Basi]/[Sýra] > 1 gefur hærra pH.',
    explanationEn:
      'To raise pH above pKa you need more base. The ratio [Base]/[Acid] > 1 gives a higher pH.',
    explanationPl:
      'Aby podnieść pH powyżej pKa, potrzebujesz więcej zasady. Proporcja [Zasada]/[Kwas] > 1 daje wyższe pH.',
  },
  {
    id: 3,
    system: 'CH₃COOH / CH₃COO⁻',
    ...
    hints: {
      ...
      method:
        'pH = pKa + log([Basi]/[Sýra]). 4.5 = 4.74 + log([Basi]/[Sýra]). log([Basi]/[Sýra]) = -0.24.',
    ...
    hintsEn: {
      ...
      method:
        'pH = pKa + log([Base]/[Acid]). 4.5 = 4.74 + log([Base]/[Acid]). log([Base]/[Acid]) = -0.24.',
    ...
    hintsPl: {
      ...
      method:
        'pH = pKa + log([Zasada]/[Kwas]). 4.5 = 4.74 + log([Zasada]/[Kwas]). log([Zasada]/[Kwas]) = -0.24.',
      solution: 'Proporcja [Zasada]/[Kwas] ≈ 0.58. Na przykład: 3 zasady i 5 kwasu → 3/5 = 0.6.',
    },
    explanation:
      'Til að lækka pH undir pKa þarf meira af sýru. Hlutfallið [Basi]/[Sýra] < 1 gefur lægra pH.',
    explanationEn:
      'To lower pH below pKa you need more acid. The ratio [Base]/[Acid] < 1 gives a lower pH.',
    explanationPl:
      'Aby obniżyć pH poniżej pKa, potrzebujesz więcej kwasu. Proporcja [Zasada]/[Kwas] < 1 daje niższe pH.',
  },
  {
    id: 4,
    system: 'H₂PO₄⁻ / HPO₄²⁻',
    ...
    hints: {
      ...
      method:
        'pH = pKa + log([Basi]/[Sýra]). 7.20 = 7.20 + log([Basi]/[Sýra]). log = 0 → hlutfall = 1.',
      solution:
        'Markmiðs-pH (7.2) = pKa (7.2). Settu jafnt af hvoru, t.d. 5 af hvoru → hlutfall = 1.0.',
    },
    hintsEn: {
      ...
      method:
        'pH = pKa + log([Base]/[Acid]). 7.20 = 7.20 + log([Base]/[Acid]). log = 0 → ratio = 1.',
      solution:
        'Target pH (7.2) = pKa (7.2). Add equal amounts of each, e.g. 5 of each → ratio = 1.0.',
    },
    hintsPl: {
      ...
      method:
        'pH = pKa + log([Zasada]/[Kwas]). 7.20 = 7.20 + log([Zasada]/[Kwas]). log = 0 → proporcja = 1.',
      solution: 'Docelowe pH (7.2) = pKa (7.2). Dodaj równe ilości, np. po 5 → proporcja = 1.0.',
    },
    explanation:
      'Fosfatstuðpúði er mikilvægur í líkamanum. Sama reglan gildir: jafnt hlutfall = pH við pKa.',
    explanationEn:
      'The phosphate buffer is important in the body. The same rule applies: equal ratio = pH at pKa.',
    explanationPl:
      'Bufor fosforanowy jest ważny w organizmie. Ta sama zasada obowiązuje: równa proporcja = pH przy pKa.',
  },
  {
    id: 5,
    system: 'H₂PO₄⁻ / HPO₄²⁻',
    ...
    hintsPl: {
      ...
      strategy:
        'pH krwi (7.4) jest nieco WYŻSZE niż pKa (7.2), więc potrzebujesz nieco więcej zasady.',
    ...
    explanation:
      'Blóð hefur pH um 7.4, sem er aðeins yfir pKa fosfats. Þess vegna er lítið meira af basa í blóði.',
    explanationEn:
      'Blood has a pH of about 7.4, which is slightly above the pKa of phosphate. That is why there is slightly more base in blood.',
    explanationPl:
      'Krew ma pH około 7.4, co jest nieco powyżej pKa fosforanu. Dlatego we krwi jest nieco więcej zasady.',
  },
  {
    id: 6,
    system: 'NH₄⁺ / NH₃',
    ...
    explanationEn:
      'The ammonium buffer works at high pH. Same idea: an equal ratio gives pH = pKa, regardless of which buffer system is used.',
    explanationPl:
      'Bufor amonowy działa przy wysokim pH. Ta sama idea: równa proporcja daje pH = pKa, niezależnie od użytego układu buforowego.',
  },
];
