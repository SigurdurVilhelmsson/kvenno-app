...
export const APPLY_PROBLEMS: ApplyProblem[] = (() => {
  ...
  out.push({
    ...
    acid: propansyra,
    concentration: 0.25,
    question: `Hvert er pH í 0,25 M lausn af ${propansyra.nameDative} (Ka = ${sci(propansyra.ka)})?`,
  ...
  out.push({
    ...
    acid: ediksyra,
    concentration: 0.1,
    question: `Basafasti Kb fyrir ${ediksyra.conjugateBaseName} (samoka basa ${ediksyra.nameGenitive}, Ka = ${sci(ediksyra.ka)}) — hver er hann?`,
  ...
  out.push({
    ...
    acid: ediksyra,
    concentration: 0.1,
    question: `Hversu hátt er klofnunarhlutfall 0,100 M lausnar af ${ediksyra.nameDative} (Ka = ${sci(ediksyra.ka)})? Svaraðu í prósentum.`,
  ...
  for (const p of APPLY_RULE_BREAKERS) {
    ...
    out.push({
      ...
      question: `Hvert er pH í ${fmt(p.concentration, 3)} M lausn af ${p.acid.nameDative} (Ka = ${sci(p.acid.ka)})? Athugaðu 5 % regluna áður en þú svarar.`,
      answer: p.answer,
      grading: { mode: 'absolute', tolerance: PH_TOLERANCE },
      answerHint: 'pH, tveir aukastafir',
      explanation:
