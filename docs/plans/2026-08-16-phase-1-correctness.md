...
| ~~**B12** GCD check~~            | **FIXED 2026-08-26.** Stilla efnajöfnur accepts non-reduced coefficient sets (`4H₂ + 2O₂ → 4H₂O` passes); the lowest-whole-number requirement is never checked or mentioned. `checkBalance` now returns `isReduced`, the level requires it, and the instructions state the rule — it was never mentioned either                                                                                                                                                                                              | `stilla-efnajofnur/src/utils/balanceChecker.ts:71-74`                                                           |
