...
14. **Each game's name disagrees with itself across three surfaces — 10 of 22.** Found 2026-09-19 while renaming `gas-law-challenge`. Compare the hub card (`apps/landing/src/pages/GamesHub.tsx`), the browser-tab `<title>` (`apps/games/{year}/{game}/index.html`) and the `gameTitle` on the `Header`; the full table is in `2026-09-19-session-record.md`. Two of the four problems are already fixed — the banned `Púfferuppskrift` (and the `.tsx?`-only scan that hid it) and the English `Gas Law Challenge`. **What needs a ruling: `molmassi` is `Mólhugtakið` on the hub and `Molmassi Leikur` in its tab (accentless), and `nafnakerfid` is `Nafnakerfið` against `Nafnapör - Efnanöfn`.** Which name wins is Siggi's, and it matters because the tab title is what a bookmark keeps. The rest is mechanical drift to sweep afterwards: mid-title capitals Icelandic does not take (`Takmarkandi Hvarfefni`, `Oxun og Afoxun`, `Lífræn Nafnagift`, `Varmafræði Spámaður`), the missing accent in `ph-titration`'s header `pH Titrun`, and three tabs suffixed `| Kvennaskólinn` / `| Kvennó` where nineteen use `- Kvennaskólinn`. **A title-agreement test would catch the whole class and cannot be written until the two names are ruled.**
