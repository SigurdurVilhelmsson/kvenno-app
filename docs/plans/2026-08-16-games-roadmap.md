...
10. ~~**Are the Ka values right?**~~ **Answered 2026-09-19.** Siggi supplied Brown Tables D.1 and D.2. Sýrufastinn's six values all match. **Appendix D is authoritative** where shipped data disagrees. **Any problem whose constant is not in Appendix D is dropped** — the known case is TRIS, which Brown does not list and which `buffer-recipe-creator` ships at two different pKa values (7.82 and 8.06). **Sýrufastinn's pool grows to 9 acids**: HF, HNO₂ and HCN are all admitted now that the naming split and the source disagreements are settled.
11. **Buffer Level 2's stored answers — FOUND 2026-09-19, needs a ruling before the Ka corrections can land.** `buffer-recipe-creator/data/problems.ts` stores `correctAcidMass`/`correctBaseMass` per problem, and **15 of 29 disagree with their own `moles × molar mass`** by 0.9 % to 182 %. `Level2.tsx:150-155` grades entered masses against those stored values at `massTolerance: 0.05`, and `:585-590` prints the false arithmetic back to the student. Level 3 is unaffected — it computes from `10^(pH − pKa)` at runtime (`Level3.tsx:87-90`). Two routes: patch the 15 values, or delete them and derive in Level 2 the way Level 3 already does. The Ka corrections touch exactly these fields, so this is decided first.
