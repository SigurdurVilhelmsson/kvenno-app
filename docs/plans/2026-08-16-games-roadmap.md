...
| ~~**Ksp / solubility equilibria**~~ **BUILT 2026-09-20**               | 3    | `3-ar/leysnijafnvaegi` — Leysnijafnvægi, chain position 7, four phases, 35 tests, registered and on the hub. **This closes Phase 5: all four confirmed gaps are now built.** **It was blocked on a source rather than on effort:** `appendix-d.ts` held D.1 and D.2 only, so no Ksp value on the platform had a source at all, and the 2026-09-19 ruling forbids shipping a constant without one. Siggi supplied Table D.3 on 2026-09-20 and it is now transcribed. **The school's two books disagree badly here** — 13 of the 33 shared compounds differ by 3× or more, worst 1538×, and a BaSO₄ molar solubility differs 14-fold — so the pool prefers the 12 that agree and every divergent salt carries a written note. **PbI₂ has no D.3 row and so cannot ship a Ksp**, the same treatment TRIS got. **Table D.3's starred sulfides are a three-species equilibrium**, not `[M²⁺][S²⁻]`, and are excluded with two tests holding it. Two defects came out of building rather than inspection: `precipitationOrder` had x and y swapped, which inverted the Mohr result the task exists to teach, and the common-ion approximation fails by 53 % on PbCl₂ — now held to `syrufastinn`'s 5 % rule so students meet one convention. See `apps/games/3-ar/leysnijafnvaegi/README.md`. ~~**The largest remaining Y3 gap is now `equilibrium-shifter`**, still entirely qualitative: general Kc/Kp and ICE tables~~ — **that gap was closed 2026-09-20 by `3-ar/jafnvaegisfasti`; see the follow-on row below**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ~~**Electrolytes, precipitation, net ionic**~~ **BUILT 2026-09-20**    | 1    | Shipped as **both**, which is what February answered and what Siggi confirmed. `1-ar/utfellingarhvorf` — Útfellingarhvörf, chain position 7 (after Stilla efnajöfnur), four phases, 60 tests, registered and on the hub; and **Stig 0 — Rafkleyfi** inside `1-ar/lausnir`, 24 tests. **The school's own textbook settled the split**: precipitation is ch. 4 (`ch04/m68710`) and electrolytes are ch. 11 (`ch11/m68781`), so the precipitation node comes first and the electrolyte phase sits later, in Lausnir. The assessment had offered two defensible chain positions and called it a teaching decision; the chapter numbers decide it without one. **Nothing in either is stored**: a scenario is two soluble salts, and the formulas, verdicts, coefficients, all three equations and the spectator ions are derived — so the ancestor's Ag₂CrO₄ defect (a chromate question against a rule table with no chromate row) is unwritable rather than fixed. The solubility table is transcribed from the book, not from the old game, whose paraphrase differed in four places. **Four terms were missing from `ordabok.md` and all four came from the textbook**: `nettójónajafna`, `heildarjónajafna`, `áhorfendajón`, `útfellingarhvarf`. See `apps/games/1-ar/utfellingarhvorf/README.md`. **Ksp is now built too** (row above) — this is the qualitative question, that one is the quantitative one, and the arc is closed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ~~**Empirical formula**~~ **BUILT 2026-09-19**                         | 1    | `1-ar/reynsluformulur` — Reynsluformúlur, chain position 5 (after Mólmassi, Brown ch. 3), four phases, 36 tests, registered and on the hub. **The three data defects were fixed first, as instructed, and all three re-verified by arithmetic before anything was ported.** The assessment was right on all three this time. One correction to it: `l2-10`'s percentages do _not_ "reduce to no whole-number formula at all" — ×3 gives 5 : 3 : 12,95, within 0,4 % of Mg₅P₃O₁₃; the arithmetic works and the chemistry does not, so `MAX_SUBSCRIPT` is what rejects them. Its conclusion (the stored `Mg₃(PO₄)₂` was unreachable) held regardless. **The class of defect is now structurally impossible:** compounds are written as formulas and both the percentages and the answer key are derived, so neither can be mistyped. See `apps/games/1-ar/reynsluformulur/README.md`. **Percent composition as a fourth `molmassi` level is still unbuilt**, and so is combustion analysis                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ~~**General Kc/Kp and ICE**~~ **BUILT 2026-09-20** (Phase 5 follow-on) | 3    | `3-ar/jafnvaegisfasti` — Jafnvægisfastinn, chain position **2**, four phases, 60 tests, registered and on the hub. Not one of the four confirmed gaps; it was named by the Ksp row above as what remained. **The chain position came from the book, not from a ruling:** ch. 13 runs Efnajafnvægi → Jafnvægisfastar → Tilfærsla jafnvægis → Jafnvægisútreikningar, so K precedes Le Chatelier, and `equilibrium-shifter`'s node label moved from `Jafnvægi` to `Hliðrun jafnvægis` now that two nodes are about equilibrium. **The 2026-09-19 Appendix D ruling has no Kc analogue** — neither book has a Kc appendix, because a general K only means anything beside its temperature — so every constant instead cites a named place in ch. 13 and `sources.test.ts` refuses one without. **Two constants carry no temperature and none was invented**; `canConvertToKp` returns false for them so they never reach the Kp exercise. The ICE solver **bisects on the extent of reaction** rather than special-casing algebra, which is why Beita can pose the N₂ + 3H₂ quartic. **A live defect was found in `equilibrium-shifter` doing it:** its Q vs K panel said "Q eykst" for any pressure increase, wrong on 14 of the 21 equilibria that offer a pressure stress, and contradicting its own bars. Two already-governed terms were finally enforced: `hvarfkvóti` → `hvarfstuðull` and `afurð` → `myndefni` (68 sites, feminine to neuter). See `apps/games/3-ar/jafnvaegisfasti/README.md`. **Followed 2026-09-20 by putting the numbers into `equilibrium-shifter` itself**, which is the gap this row named: the engine moved to `packages/shared`, 20 of its 30 systems took a constant the platform already sources, ΔH is derived from the book's formation enthalpies rather than stored, and van 't Hoff makes the temperature stress move K. A second live defect surfaced doing it — the game shipped the enthalpy of **solid**-iodine HI formation against a gas-phase equation, so both temperature answers were backwards on a beginner system. See the CLAUDE.md entry |

Also unplaced: percent yield. ~~significant figures~~ **built 2026-09-19** as a Stig 0 inside `dimensional-analysis` — see item 7 above. One qualification, from the Level-3 measurement behind Phase 1 item 3: in Level 3 the significant-figures check is feedback-only — computed at `Level3.tsx:159-162`, rendered as a panel at `:745-770`, and never passed to `calculateCompositeScore`. Whether Levels 1–2 actually grade on it was not measured; confirm before treating "grades" as established. The teaching gap is real either way. **The mass→mole→mass bridge, listed here as unplaced until 2026-08-26, is now built** — `1-ar/einingakedjan`, see the status block at the top. Percent yield is still unplaced.
...
7. ~~**Where significant figures live**~~ **Answered and built 2026-09-19: a Stig 0 inside Einingagreining** — which is `1-ar/dimensional-analysis` (`GamesHub.tsx:31`), **not** `1-ar/einingakedjan`. Four steps: the four counting rules worked through, counting practice, writing to a given precision, then the two arithmetic rules. No score, no timer. The gap was real and this was the right home: `Level3.tsx:167-169` already marked students on significant figures and nothing taught them. **The open question of whether Levels 1–2 grade on it is settled — they do not mention it at all.** `utils/sigfigs.ts` works on the written string rather than a `number`, because `1200`, `1200,` and `1,200 × 10³` are one quantity at three different precisions; a round-trip property test (455 cases) found that 60,221 to two figures cannot be written in plain decimal, so the renderer verifies itself and falls back to `6,0 × 10¹`.
8. ~~**Acid names the platform spells three ways**~~ **Answered 2026-09-19 — see item 9, which is this item's answer.** Surfaced 2026-09-03 while picking the Sýrufastinn pool, and the reason three acids are absent from it. **HF:** `Flússýra` (`3-ar/ph-titration/data/titrations.ts:87`) vs `flúorsýru` (same game, `level2-puzzles.ts:82`, `level3-challenges.ts:303`) vs `Flúorsýra` (`2-ar/intermolecular-forces/components/Level1.tsx:426`) — two words inside one game. **HNO₃/HNO₂:** `saltpéturssýra`, `saltpétursýru` (one `s`), `salpeturssýru` (missing `t` and accent, `3-ar/equilibrium-shifter/data/equilibria.ts:415`). **H₃PO₄:** `fosforsýru` at `equilibria.ts:856` is not a new question but a **missed site of the existing B5 `Fosfór` ruling** and can be fixed without one. Also `brennisteinsýru` with one `s` (`2-ar/hess-law/components/Level2.tsx:177`) and `Benzoesýrustuðpúði` with a `z` (`3-ar/buffer-recipe-creator/data/problems.ts:146`).
9. ~~**Acid names the platform spells three ways**~~ **Answered 2026-09-19** — see item 8; `flússýra`, `saltpéturssýra`, `brennisteinssýra`, `bensósýra`, `fosfórsýra`, all five now in `ordabok.md` and enforced by `governed-terms.test.ts`.
10. ~~**Are the Ka values right?**~~ **Answered 2026-09-19.** Siggi supplied Brown Tables D.1 and D.2. Sýrufastinn's six values all match. **Appendix D is authoritative** where shipped data disagrees. **Any problem whose constant is not in Appendix D is dropped** — the known case is TRIS, which Brown does not list and which `buffer-recipe-creator` ships at two different pKa values (7.82 and 8.06). **Sýrufastinn's pool is 9** — HF landed 2026-09-19, and HNO₂ and HCN followed the same day once Siggi ruled the four Icelandic terms that had blocked them (`saltpéturssýrlingur`, `nítrítjón`, `vetnissýaníð`/`blásýra`, `sýaníðjón`, all now in `ordabok.md`). See `2026-09-19-session-record.md`.
11. ~~**Buffer Level 2's stored answers**~~ **Answered and built 2026-09-19: derive.** The finding was real — `buffer-recipe-creator/data/problems.ts` stored `correctAcidMass`/`correctBaseMass`/moles/`ratio`, and **13 of the 29 real problems** (not 15; the first audit mis-flagged #25, which is correct under its own `phAdjustment` model) had at least one value that disagreed with the others. Level 2 graded entered masses against them at ±5 %, so **three of the six problems it served** (#14, #17, #19) marked a correct student wrong, and the explanation panel printed the false arithmetic back. `engine/buffer.ts` now derives everything and the five fields are gone from the type and the data; Level 3 was never affected. **`phAdjustment` is the branch to preserve** — problem #25 weighs out all the acid and adds NaOH, a model that lived only in the stored numbers because `Level2.tsx` never read the flag.

12. ~~**The i18n question**~~ **Answered 2026-09-19, and built the same day: strip the switcher where it translates nothing.** The eight that routed essentially nothing through `t()` are stripped — seven with **zero** calls (`2-ar/kinetics`, `2-ar/lewis-structures`, `2-ar/intermolecular-forces`, `2-ar/organic-nomenclature`, `3-ar/buffer-recipe-creator`, `3-ar/gas-law-challenge`, `3-ar/thermodynamics-predictor`) plus `3-ar/ph-titration`, whose one call was a fallback-backed lookup over five badge labels. 1 661 lines of dead `i18n.ts` deleted, zero consumers. `packages/shared/i18n/__tests__/switcher-earns-its-place.test.ts` states the rule as a property — a game rendering `LanguageSwitcher` must have at least one `t()` call — so the next half-wiring fails rather than ships. **Still open, deliberately:** the barely-translated three, `2-ar/rafeindabygging` (1 call, but a real one — `game.title`, not a fallback), `2-ar/vsepr-geometry` (2) and `1-ar/takmarkandi` (2). The ruling drew the line at zero; these are above it. Per-game counts: `docs/i18n-coverage.md`.

13. **Trim Sýrufastinn's Beita phase — ruled 2026-09-19, not yet built.** Adding HNO₂ took it from 12 problems to 16, and twelve of those are one generated template differing only in acid and concentration. Siggi ruled that it should be trimmed; **the selection rule is his and has not been given.** `APPLY_PROBLEMS` in `3-ar/syrufastinn/src/data/problems.ts` appends every member of `RULE_BREAKING_PROBLEMS` — that is the one line. Unruled candidates: one pair per rule-breaking acid (3), a spread across the α range, or a cap on the widest margins. Constraints to keep whatever is chosen: Æfa stays entirely inside the 5 % rule and Beita stays partly outside it (both asserted by `problems.test.ts`), and at least one pair should survive from each of HF, maurasýra and saltpéturssýrlingur, which fail the approximation by visibly different margins.

14. ~~**Each game's name disagrees with itself across three surfaces — 10 of 22.**~~ **Answered and built 2026-09-19.** Siggi ruled the two real conflicts — **`Mólhugtakið`** for `molmassi` and **`Nafnakerfið`** for `nafnakerfid`, the hub-card name both times — which unblocked the rest. All ten fixed: the banned `Púfferuppskrift` (and the `.tsx?`-only scan that hid it), the English `Gas Law Challenge`, the two two-name games, and the drift behind them (mid-title capitals, `pH Titrun`'s missing accent, three `| Kvennaskólinn` suffixes). `packages/shared/i18n/__tests__/game-titles-agree.test.ts` now enforces hub card = browser tab = in-game header, with the hub card as the source of truth; `2-ar/rafeindabygging` is the one header exemption, for per-screen titles, and the test checks that waiver is still earned.
