...
**Year 1:** dimensional-analysis, lotukerfid, nafnakerfid, molmassi, reynsluformulur, stilla-efnajofnur, utfellingarhvorf, takmarkandi, lausnir, einingakedjan
...
Every game screen was made playable at 360 px by touch in September 2026. What that leaves behind:

- **Touch defaults live in `packages/shared/styles/game-base.css`**, in the `base` and `utilities`
  layers so any utility class still wins: `overflow-wrap: break-word` on `body`,
  `touch-action: manipulation` on every control, `min-width: 0` on form fields, and 16 px field
  text on touch so iOS does not zoom on focus. Do not repeat them per game. A mid-word break at
  360 px is a layout defect to fix, not something to live with.
- **Touch-only growth uses `pointer-coarse:`** (`pointer-coarse:min-h-11`,
  `pointer-coarse:py-3 pointer-coarse:-my-3`), which leaves desktop pixel-identical; `sm:`/`md:`
  are for layout. Hover effects are gated to `(hover: hover) and (pointer: fine)`.
- **`e2e/mobile-games.spec.ts` replays every path in `e2e/mobile-game-screens.ts`** at 360 px
  and fails on sideways page scroll; Chromium runs all ~290 paths, Firefox a sample (each menu and
...
- **B12, non-reduced coefficients.** `checkBalance` returns `isReduced` next to `isBalanced` and the level requires both, so `4H₂ + 2O₂ → 4H₂O` is no longer correct. The review's finding was that the convention is never checked **or mentioned**, so the instructions now state it and balanced-but-unreduced gets its own feedback instead of a bare "Rangt". Guarded by `1-ar/stilla-efnajofnur/src/__tests__/balance-checker.test.ts` — this game's **first** tests, which is how B12 survived four review iterations; it also verifies all 20 answer keys are balanced and reduced.
...
| enthalpy              | `vermi`       | `skammtavarmi`, `enþalpía`, `entalpía` | **Not masculine, whatever this note said until 2026-09-23** — the book agrees it as a neuter (`gufunarvermið`, `hátt brunavermi`, gen. `-vermis`), so check agreement when swapping it in. `enþalpía` is an old-repo `calorimetry` coinage — do not import it if that game is ported; `entalpía` is its t-spelling, found in `thermodynamics-predictor`                                                                                                   |
...
| tetrahedron | `ferflötungur` / `ferflötungslaga` | `fjórflötungur` | **2026-09-23, rule 1**: `ordabok.md` gives both (`tetrahedral;ferflötungur`, `tetrahedral;ferflötungslaga`), and so does the book's own glossary — the headword in `ch07/m68742`, the VSEPR module, is `ferflötungur`, whose paragraphs write `fer-` 19 times to 8. **Siggi's to confirm, not blocking:** the corpus overall is 84 `fjór-` to 57 `fer-`, and that module's one inline term marker is `fjórflötunga`. Masculine either way, so the sweep of `vsepr-geometry` and `intermolecular-forces` was a prefix swap |
...
does not decline: not `samoki basinn`), `hvarfgangur` (not `hvarfgangsháttur`), `rafeindaskipan`
(not `rafeindauppsetning`), `hliðarmálmur` (not `skiptimálmur`), `málmleysingi` (not `ómálmur`),
`myndunarvermi` (not `myndunarvarmi`) and `útvermið`/`innvermið` (not `exóþerm`/`endóþerm`). Two
of them, `hvarfgangur` and `rafeindaskipan`, survived only on the games-hub cards, which no
...
the glossary exists to prevent; say so if that was not wanted.

**The identifier followed on 2026-09-24, Siggi's call.** The August sweep left the folder, URL slug
and progress key as `jafna-jofnur` / `jafnaJofnurProgress` so as not to break links or wipe progress.
They are now `apps/games/1-ar/stilla-efnajofnur`, `/efnafraedi/1-ar/games/stilla-efnajofnur.html`
and `stillaEfnajofnurProgress`, with both costs paid for rather than accepted:
`server/nginx-site.conf` 301-redirects the old URL, and `useGameProgress` takes a `legacyKeys`
argument that carries saved progress over from `jafnaJofnurProgress` once and removes it. **Keep
the redirect**; links in old class material outlive any deploy. The same two steps apply to any
future game rename.
