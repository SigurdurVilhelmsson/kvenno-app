...
**Fixed 2026-08-26** — Lausnir's weighing premise. Each of the 21 chemicals now declares a `form`, and the two problem types that say "Þú leysir …" draw only solids that can be weighed out. The liquids and the one gas keep their dilution, mixing and massFromMolarity problems, which is where diluting concentrated acid from stock belongs. **The recorded note was right about the seven substances and wrong about the masses:** only `Ca(OH)₂` reached milligrams (16 mg floor, against a 0.81 g ceiling set by its 0.022 M saturation); `K₂Cr₂O₇` bottoms out at 1.44 g, comfortably weighable. Guarded by `1-ar/lausnir/src/__tests__/weighable-substances.test.ts`. The one item left from that note is a stocking question, not a code defect: **K₂Cr₂O₇ is Cr(VI), a category-1 carcinogen** many Icelandic schools no longer hold — whether it stays in the pool is Siggi's call.
